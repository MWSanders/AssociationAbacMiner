...
class ParallelHashEnumerator(object):
    def __init__(self, pool, possible_params, statement, log_universe_generator, producer_threads=8, use_resources=True, param_info=None, event_normalizer=None):
        ...
        for user in user_enumerator.generate_events(pool):
    ...
    def single_threaded_calc_hashest(self, user_events, op_events, service_ops_types):
        hashes_list = []
        dicts_list = []
    ...
    def calculate_allowed_hashes(self, pool, callback):
        ...
        if self.producer_threads == 1:
            return self.single_threaded_calc_hashest(self.user_events, self.op_events, self.service_ops_types)
        ...
        for data_list in resource_iters:
            if largest_type == 'users':
                input_data_lists.append((data_list, self.op_events, self.service_ops_types))
            elif largest_type == 'ops':
                input_data_lists.append((self.user_events, data_list, self.service_ops_types))
